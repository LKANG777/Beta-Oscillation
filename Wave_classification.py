...
# Excitatory Current 
data_current = np.loadtxt(filename+"_current_tau_fix_e.txt",delimiter=',',skiprows=n_skiprows,usecols=range((N-2*fix_width)*(N-2*fix_width)))
data_current1=np.delete(data_current,delete_list,axis=1) #Data shape [T]*[10*10]
# Common external input
data_external_input= np.loadtxt(filename+"_current_tau_fix_xi_g.txt",delimiter=',' ,skiprows=n_skiprows,usecols=range((N-2*fix_width)*(N-2*fix_width)))
